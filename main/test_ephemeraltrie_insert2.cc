...
using trie_t = trie::AtomicTrie<trie::EmptyValue, trie::UInt64Prefix, trie::EphemeralTrieMetadataBase, 5, 2>;
...
void *insert_worker1(void *ptr) {
    ...
    assert(ref.insert(trie::UInt64Prefix{0x0}));
    return NULL;
}

void *insert_worker2(void *ptr) {
    trie::AtomicTrieReference ref(*t);
    assert(ref.insert(trie::UInt64Prefix{0x1}));
    return NULL;
}
void *insert_worker3(void *ptr) {
    trie::AtomicTrieReference ref(*t);
    assert(ref.insert(trie::UInt64Prefix{0x10}));
    return NULL;
}
...
void imc_check_main(void) {
    ...
    imcthread_create(&t3, NULL, insert_worker3, 0);

    imcthread_join(t1, NULL);
    imcthread_join(t2, NULL);
    imcthread_join(t3, NULL);

    assert(t->deep_sizecheck() == 3);
}
